...
function App() {
  ...
  {"ques":"Why was the camea company called Kodak","ans":'The company, Kodak, was founded by George Eastman in 1888. When asked, Eastman claimed that the word “Kodak” had no meaning behind it. He said that the letter K seemed “a strong, incisive sort of letter.”'}];
  const [card, setCard] = useState(facts[0]);

  const [flipped, setFlipped] = useState(false);
  
  const updateCard = (e) => {
    e.preventDefault();
    var d = Math.floor(Math.random()*facts.length);
    if (card==facts[d]){
      d+1==facts.length ? setCard(facts[0]) : setCard(facts[d+1])
    }
    else{
      setCard(facts[d]);
    }
